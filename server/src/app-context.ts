...
import { FfmpegMediaTools } from './media/ffmpeg.js';
...
export function createAppContext(config: AppConfig, overrides: Partial<AppContext> = {}): AppContext {
  ...
  const media = overrides.media ?? new FfmpegMediaTools();
  const video = overrides.video ?? createVideoClient(config, logger, media);
  const text = overrides.text ?? createTextClient(config, logger);
  const planner = overrides.planner ?? createPlanner(config, text, logger);
  const pipeline =
    overrides.pipeline ?? new GenerationPipeline({ config, repo, video, planner, storage, media, logger });
