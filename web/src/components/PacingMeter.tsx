...
const TONE: Record<PacingLevel, { text: string; bar: string; label: string }> = {
  ...
  tight: { text: 'text-warn', bar: 'bg-warn', label: 'A bit long' },
  over: { text: 'text-danger', bar: 'bg-danger', label: 'Too long' },
};

/** Estimated spoken length vs the target duration (green fits, amber is tight, red is too long). */
    ...
    <div id={id} className="space-y-1.5">
      <div className="flex flex-wrap items-baseline justify-between gap-x-3 gap-y-1 text-xs">
        ...
        <span className="text-muted tabular-nums">
          {pacing.words} {pacing.words === 1 ? 'word' : 'words'} · about {pacing.seconds.toFixed(1)}s of{' '}
          {pacing.targetSeconds}s
        </span>
      </div>
      <div
        ...
        aria-valuemin={0}
        aria-valuemax={pacing.targetSeconds}
        aria-valuenow={Math.min(pacing.seconds, pacing.targetSeconds)}
        aria-valuetext={`About ${pacing.seconds.toFixed(1)} seconds of ${pacing.targetSeconds}`}
      >
        <div
...
export function PacingInline({ pacing }: { pacing: Pacing }) {
  ...
  return (
    <span className={cn('text-xs tabular-nums', tone.text)}>
      about {pacing.seconds.toFixed(1)}s of {pacing.targetSeconds}s
    </span>
  );
}
